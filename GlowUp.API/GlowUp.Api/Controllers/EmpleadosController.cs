...
[Authorize, ApiController, Route("api/empleados")]
public sealed class EmpleadosController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<EmpleadoResponse>>> Buscar([FromQuery] long negocioId, [FromQuery] bool incluirInactivos, CancellationToken cancellationToken)
    {
    ...
    [HttpGet("{id:long}")]
    public async Task<ActionResult<EmpleadoResponse>> Obtener(long id, CancellationToken cancellationToken)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
    ...
    [HttpPost]
    public async Task<ActionResult<EmpleadoResponse>> Crear(GuardarEmpleadoRequest request, CancellationToken cancellationToken)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
    ...
    [HttpPut("{id:long}")]
    public async Task<ActionResult<EmpleadoResponse>> Actualizar(long id, GuardarEmpleadoRequest request, CancellationToken cancellationToken)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
